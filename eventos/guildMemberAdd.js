module.exports = {
	run: async function(member) {
		...
		let msg = await channel.send(`» Olá **${message.author.username}** | Esta é uma mensagem para nós verificarmos se você é um rôbo, caso não seja um robô clique no emoji \`\`<:correto:505155063963058187>\`\`, ou se você quiser sair do servidor clique no emoji \`\`<:negado:505155029636874250>\`\``);
			...
			switch(r.emoji.id) {
				case "505155063963058187":
						...
						if (roleVerified) {
							...
							channel.delete();
							member.send(`» **${message.author.username}** | Você foi verificado com sucesso! Agora você pode interagir no servidor.😜`);
						}
				break;
				case "505155029636874250":
					member.send(`» **${message.author.username}** | Que pena que não quis se juntar ao nosso servidor, espero que um dia você volte.😜`)
